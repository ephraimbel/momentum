...
- **Gate 1: substantially met.** Both live loops run start→finish→summary→share; history present; Reduce-Motion handled in `IridescentView`/components. Remaining before formally closing Gate 1: VoiceOver label pass on set rows/rest timer/map; jank check at 1000+ history items; and the device-only live-map ±2% validation. Deferred within Phase 1 (tracked, non-blocking): anatomical muscle-map diagram, supersets UI, RPE polish, inline set reorder/swap, true-B/W route snapshot, map "bloom" transition, weekday-aligned heatmap, PersonalRecord persistence.
