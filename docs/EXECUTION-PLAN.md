...
- ✅ **Slice 2 (landed):** other profiles + follow. `CommunityDirectory` (athletes w/ stats + posts — one source for feed + profile); `FollowStore` (@Observable, persisted handle graph, app-injected); `AthleteProfileView` (identity + Momentum badge + Follow button + stats + recent posts); World feed authors navigate to profiles, Following segment shows followed athletes. 195 unit + 9 UI green; `testFollowAthleteSurfacesInFollowing` drives feed→profile→Follow→Following. Next: Slice 3 (the globe). Live multi-user parts (real feeds/globe/presence) light up with Supabase, like sync/AI.
