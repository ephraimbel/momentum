...
- ✅ **Slice 5 (landed):** **moderation** — block + report (comments still deferred until backend). `ModerationStore` (@Observable, persisted block-handles + reported-post-ids); `FeedPostCard` context menu (Report w/ reasons / Block) + `AthleteProfileView` ⋯ menu (Report / Block + unfollow + pop); feed + globe filter blocked/reported content. `ReportReason` enum (App Store UGC). DEBUG `--reset-social` isolates the social UI tests. 208 unit + 12 UI green; `testBlockRemovesAthleteFromFeed` verifies. **Social layer feature-complete in-sandbox.** Remaining: comments (need moderation + backend), and the live multi-user/realtime + city/street MapKit zoom — all light up with Supabase, like sync/AI.
