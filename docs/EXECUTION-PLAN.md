...
- ✅ **Strength live loop end-to-end:** `StrengthViewModel` (bridges the engine actor + durable store) → `StrengthLiveView` (set rows w/ ghosted previous + one-tap ✓, auto rest-timer ring, plate calculator) → `StrengthSummaryView` (volume/sets/duration, PR badges via `StrengthPRs`, working-sets-by-muscle). Today `Start` → `SportPicker` → strength live → summary. *(The interim `ActivityChooserView` was superseded by `SportPicker` and deleted in the 2026-07-10 cleanup.)*
