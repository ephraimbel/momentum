...
- ✅ **Slice 4 (landed):** **respect reactions** + the **live-presence seam**. `ReactionStore` (@Observable, persisted, app-injected); one iridescent "respect" on `FeedPostCard` (count = labeled baseline + viewer's real reaction). `PresenceServing` seam (`StubPresenceService` 0 / config-gated `LivePresenceService` 0 until Supabase Realtime) → globe shows "· N live now" only when real presence > 0 (never fabricated). 204 unit + 11 UI green; `testRespectReaction` + screenshot verify. **Remaining: Slice 5 (moderation tooling + optional comments).** Live multi-user/realtime + city/street MapKit zoom light up with Supabase, like sync/AI.
