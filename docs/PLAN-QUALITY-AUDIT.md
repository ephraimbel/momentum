...
9. **Any timeframe, honestly** — **SHIPPED 2026-07-10** (extended same day to full seasons). Plans generate for any runway: a race in days gets a real 1-week race-week block; ≤3 weeks out becomes an all-taper "get you to the line fresh" block holding volume *below* current load with one race-pace touch a week (previously a race 2 weeks out generated a 4-week plan extending past race day). **A marathon next year generates the whole 52-week season** — ~13 weeks base, months of build with deloads, peak, and the distance taper landing exactly on race week — with volume growing toward the *race-appropriate readiness peak* (`PlanFeasibility.peakWeeklyVolumeM`, clamped ≤3.5× start) and then holding, instead of an arbitrary 2× cap. Races beyond 52 weeks get pure foundation with no phantom taper. The feasibility `tooShort` verdict explicitly promises day-by-day coaching to the start line alongside the honest "you can't get race-ready in this window." Intensity remains the athlete's dial (gentle/balanced/aggressive regenerates the season; injury caps stay protective).
