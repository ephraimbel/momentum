...
9. **Any timeframe, honestly** — **SHIPPED 2026-07-10.** Plans now generate for any runway: a race in days gets a real 1-week race-week block; ≤3 weeks out becomes an all-taper "get you to the line fresh" block that holds volume *below* current load with one race-pace touch a week (previously a race 2 weeks out generated a 4-week plan extending past race day). The horizon rose 16→24 weeks, and a race *beyond* the window generates pure foundation — base/build/deloads with no phantom taper (the race block arrives on regeneration closer in). Long ramps plateau at ≤2× starting volume. The feasibility `tooShort` verdict now explicitly promises day-by-day coaching to the start line alongside the honest "you can't get race-ready in this window."
