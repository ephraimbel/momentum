...
- 2026-09-07 (night) — responsiveness pass. `PerfMark` (DEBUG) logs tap-to-screen per flow; `NewFlowsBenchmarkTests` times the engine (marathon preview 3 ms, rebuild proposal 18 ms, plan switch 75 ms, fuel readout over 500 meals 11 ms); `NewFlowsPerfUITests` drives the screens. Trimmed the waste around the engine (Manage availability and proposal after the sheet lands; the shelf's current-plan review built once at the tap; undo-point fetch by predicate; the builder given fetched workouts; schedule skips a redundant generator run; step crossfades; the Plan tab settles once per day and re-derives once per dismissal; Fuel refresh de-duplicated, usuals behind their own token, resolver decodes only scalar-less rows, bounded water query, no layout animation on a day switch). Simulator: Fuel day switch 89 → 18 ms, Your plans 209 → 168 ms, builder 150 ms, proposal ready ~300 ms including a deliberate 150 ms settle. Unit 2,185 / 0 failures; 24 UI green.
