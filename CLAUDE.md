...
## What this is
A premium **running-first adaptive endurance trainer** for iOS — *from your first 5K to your first ultra* (pivot 2026-07, see ENDURANCE-FOCUS.md). The moat is **honest + adaptive**: a deterministic plan engine wrapped in recovery-aware adaptation (HealthKit is the single wearable integration — Oura/Garmin/Whoop/Watch **signals** all arrive through it; see the Health rule below), an injury feedback loop with gated return, and truthful time-to-goal verdicts (`PlanFeasibility`). **Strength stays as a supporting pillar** (strength-for-runners), and other sports remain trackable — but running is the headline. Fueling, not dieting; never a diagnosis. Social exists as **honest presence** (opt-in, private-by-default; `docs/SOCIAL-LAYER.md`) — no feed/kudos mechanics.

One-liner: **keep moving.**

## Non-negotiable principles
- **Apple-native UI.** SwiftUI, iOS 18+, HealthKit; no third-party **UI** SDKs. **Maps are Mapbox** (decision 2026-06-16, overrides the original MapKit-only rule) — all map rendering uses the MapboxMaps SDK via `MapStyleOption`/`RouteMapView`/`HeatmapMapView` (`MBXAccessToken` pk-token in Info.plist; SDK download via a `~/.netrc` secret token, not committed). *Exception: watchOS — Mapbox has no watchOS SDK, so the watch app's live/summary maps use MapKit (2026-07-09).* Allowed third-party deps: MapboxMaps, RevenueCat, Superwall, supabase-swift, TikTokBusinessSDK (ads attribution only — SKAN, no ATT; added 2026-08-11).
- **Monochrome + iridescent.** ~95% pure black/white. Iridescence is an *earned* accent — it appears ONLY on progress/achievement (rings, PRs, streaks, live route accent, rest-timer ring, plan reveal). If it's not marking progress, it's not there. Dark mode is a soft warm charcoal, not true black (decision 2026-07-10).
- **Apple Health is a source of signals, never a source of workouts** (owner call 2026-08-15). Connecting Health connects, and that is all: it reads no history, imports nothing, and creates **no `Workout` rows, ever**. We read sleep, HRV, resting/walking HR, respiratory rate, wrist temperature, VO₂max, body mass and steps, and the recovery picture builds up day by day from the moment the athlete connects. The importer was deleted (`d419f0f`) after a fresh signup pulled 10,000 mirrored Watch/Garmin/Strava rows and made the app unusable — a Garmin-recorded run no longer appears in Momentum at all, and that is intended. The one workout read that survives is `HealthService.workoutSpans(from:to:)`, which returns time *windows* only so a session isn't double-counted as incidental movement; never let it grow back into an import.
...
## Decisions made
- **Design direction (2026-06-09): LIGHT/white is the hero aesthetic** (overrides the PRD's dark-hero §5.1). Clean white canvas, faint-gray cards, near-black ink, soft iridescent gradient accents. **Dark mode shipped 2026-07-10**: a first-class night look — softened same day from true black to a warm charcoal (background #1E1D1B, surface #2A2926; "Claude dark" feel, pure black felt too aggressive) — Settings → Appearance offers System/Light/Dark (default System, `AppAppearance` + `preferredColorScheme` in `MomentumApp`; the `.light` force is gone). All color routes through `Theme` tokens so both palettes come from one place; adaptive map pairing follows appearance (Realistic→night preset, Light basemap→Dark) while deliberate styles render as chosen. Physical objects (medallion badges) keep fixed materials — engraving ink never re-anodizes. The in-app brand element is **`BrandMark`** — the app icon (glass runner, `BrandIcon` imageset) clipped to app-icon corners (decision 2026-07-10; the `IridescentOrb` is retired and deleted — do not reintroduce it). The **app icon is the glass runner** (`Assets.xcassets/AppIcon.appiconset/icon-1024.png`; the old M/orb generator scripts were deleted 2026-07-10; regenerate route data with `scripts/fetch_community_routes.py`). **Typography (2026-06-09): two bundled open-source faces (OFL), no longer SF Rounded** — **Space Grotesk** is the display face (`Font.display` → hero numbers, titles, wordmark; technical/strong, tabular figures) and **Inter** is the UI face (`Font.rounded` → all body/labels/buttons; clean neutral sans). Both live in `Momentum/Resources/Fonts/`, registered via `UIAppFonts` in `project.yml`. All type routes through the two helpers in `Typography.swift`, so the whole app re-types from that one file — to change faces, edit only the PostScript names in `BrandFont`. `Image(systemName:)` icons stay on `.system()`. (Static weight files were sliced from the variable fonts with `fontTools.varLib.instancer`.)
- **Tab layout (as of 2026-07-16): Today · Plan · Progress · Fuel · Profile** — the bar is full (5 is the iOS max). Today is map-centric — edge-to-edge map, floating glass header, deck of three thoughts (plan → Start → one utility line). **There is no History tab** — history is a segment inside Progress (Trends · History · You), with the personal heatmap as a look-back card at its top (a card, never a tab — decided 2026-06). **Fuel is the meal-tracking + fueling-readiness page** (`FuelView` + `FuelReadiness` engine — floors never ceilings, no dieting). **Community is BACK-BURNERED from v1 (2026-07-16)** — the app ships solo-first ("Bevel for endurance athletes"); the feed/backend/stores stay in the repo dormant (docs/SOCIAL-LAYER.md) and return when a real user base exists. No visibility controls ship on save screens; the profile trio is Workouts · Miles · PRs (no followers). Profile is a TikTok-style grid of workout tiles. Plan is a date-badge planner with macrocycle phase chips.
- **Brand accents (rebrand "Lavender Glass", 2026-08-16):** **`Theme.purple` (asset-backed: #7C63F0 light / #9D8BF5 dark — the board's continuity rule) is the brand lavender — the app-wide interactive accent**: it is the global tint (`MomentumApp`; links, toggles, pickers, selected tab icon), the `SegmentedCapsule` selection pill, and still the Pro/marketing surface (paywall, puck, injury banner). Scarcity is the discipline — lavender means "tappable or happening now", never decoration; primary CTAs stay ink pills. Iridescence stays earned-only but is retuned to a **lavender-led aurora** (`Theme.iridescent`: lavender/sky/rose/violet/orchid — mint and peach are out). **Fuel is the one vivid room**: `Theme.Fuel` wears the fruit palette (citrus/blueberry/raspberry/lagoon/rooibos/kiwi, single hexes validated on both surfaces) and lavender never enters Fuel. In strength contexts the anatomical `MuscleMap` replaces the orb.
