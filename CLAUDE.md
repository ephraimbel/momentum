...
- **Apple-native UI.** SwiftUI, iOS 18+, HealthKit; no third-party **UI** SDKs. **Maps are Mapbox** (decision 2026-06-16, overrides the original MapKit-only rule) — all map rendering uses the MapboxMaps SDK via `MapStyleOption`/`RouteMapView`/`HeatmapMapView` (`MBXAccessToken` pk-token in Info.plist; SDK download via a `~/.netrc` secret token, not committed). *Exception: watchOS — Mapbox has no watchOS SDK, so the watch app's live/summary maps use MapKit (2026-07-09).* Allowed third-party deps: MapboxMaps, RevenueCat, Superwall, supabase-swift.
