...
struct TodayView: View {
    ...
    var body: some View {
        ...
        .workoutRunner(launch: $launch, offersRecovery: true)
    ...
    private var activitySelector: some View {
        ...
        // `highPriorityGesture` so the tap wins over the live map's gesture recognizer instantly — a
        // plain Button loses the race over Mapbox and the picker feels delayed (same fix as `worldButton`).
        .highPriorityGesture(TapGesture().onEnded { Haptics.light(); showSportPicker = true })
        .accessibilityElement()
        .accessibilityLabel(activity.title)
        .accessibilityHint("Choose activity")
        .accessibilityAddTraits(.isButton)
    }
...
/// A streak chip — flame + count; lights up iridescent when the streak is alive. The symbol gently
    ...
    var body: some View {
        HStack(spacing: 6) {
            Image(systemName: "flame.fill")
                .font(.system(size: Theme.FontSize.body - 1, weight: .bold))
