...
struct RunAnalysisSection: View {
    ...
    /// Max HR for zone banding (Tanaka estimate or measured, from the profile). nil → HR line only,
    /// no zone distribution.
    var maxHR: Int? = nil
    ...
    /// Stored heart-rate readings as elapsed-time points (seconds from the first reading).
            ...
            VStack(alignment: .leading, spacing: Theme.Space.lg) {
                ...
                if hr.count >= 4 {
                    ...
                    if let maxHR, maxHR > 0 {
                        let zones = HeartRateZones.distribution(bpms: hr.map(\.bpm), maxHR: maxHR)
                        if !zones.isEmpty { zonesCard(zones, spanS: hr.last!.t - hr.first!.t) }
                    }
                }
    ...
    /// Time-in-zone bars (Z1…Z5) from the stored readings — the post-run distribution the zone chip
    /// summarized live. Monochrome; the dominant zone reads strongest in ink, no iridescence (a zone
    /// split is information, not an achievement).
    private func zonesCard(_ zones: [Double], spanS: TimeInterval) -> some View {
        let top = zones.max() ?? 0
        return chartCard("Zones", subtitle: "Time in zone") {
            VStack(spacing: Theme.Space.sm) {
                ForEach(1...HeartRateZones.count, id: \.self) { z in
                    let fraction = zones[z - 1]
                    HStack(spacing: Theme.Space.sm) {
                        Text("Z\(z)")
                            .font(.rounded(11, weight: .bold)).monospacedDigit()
                            .foregroundStyle(Theme.inkSecondary)
                            .frame(width: 24, alignment: .leading)
                        Text(HeartRateZones.label(z))
                        ...
                        GeometryReader { geo in
                            ZStack(alignment: .leading) {
                                ...
                                if fraction > 0 {
                                    Capsule()
                                        .fill(fraction == top ? Theme.ink : Theme.ink.opacity(0.45))
                                        .frame(width: max(6, geo.size.width * fraction))
                                }
                            }
                        }
                        .frame(height: 8)
                        Text(Formatters.duration(s: spanS * fraction))
                            .font(.rounded(11, weight: .semibold)).monospacedDigit()
                            .foregroundStyle(Theme.inkSecondary)
                            .frame(width: 48, alignment: .trailing)
                    }
                    .accessibilityElement(children: .ignore)
                    .accessibilityLabel("Zone \(z), \(HeartRateZones.label(z)), \(Int((fraction * 100).rounded())) percent")
                }
            }
        }
    }
