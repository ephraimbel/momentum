...
struct CardioTrackingView: View {
        ...
        VStack {
            HStack {
                ...
                if phase == .tracking, let vm {
                    HStack(spacing: 4) {
                        Image(systemName: vm.gpsLost ? "location.slash.fill" : "location.fill")
                        Text(vm.gpsLost ? "Reconnecting…"
                    ...
                    .animation(Motion.standard, value: vm.gpsLost)
                }
            }
                ...
                TimelineView(.periodic(from: vm.startedAt, by: 1)) { ctx in
                    VStack(spacing: Theme.Space.sm) {
                        HStack(spacing: Theme.Space.xl) {
                            ...
                            // Live heart rate — only with a live source (Watch in a workout / strap).
                            if let bpm = vm.bpm {
                                stat("\(bpm)", "bpm")
                            }
                        }
                        if let zone = vm.bpmZone { zoneChip(zone) }
                    }
                }
    ...
    /// The current HR training zone as a quiet monochrome chip — "Z3 · TEMPO". Informational, not an
    /// achievement, so it never earns iridescence.
    private func zoneChip(_ zone: Int) -> some View {
        Text("Z\(zone) · \(HeartRateZones.label(zone).uppercased())")
            ...
            .accessibilityLabel("Heart rate zone \(zone), \(HeartRateZones.label(zone))")
    }
