...
enum CommunityGenerator {
    ...
    private static func athlete(index i: Int, now: Date) -> CommunityAthlete {
        ...
        let lat = city.lat + rng.double(-0.05, 0.05)
        let lon = city.lon + rng.double(-0.05, 0.05)
        ...
        let post = makePost(index: i, name: name, handle: handle, city: city.name,
                            discipline: discipline, lat: lat, lon: lon, date: postDate, rng: &rng)
    ...
    /// A short, realistic-looking loop (~0.6–2 km) near the start — real coordinates so the feed map
    /// frames an actual place behind the trace.
    static func loopRoute(lat: Double, lon: Double, rng: inout SeededRNG) -> [[Double]] {
        let radius = rng.double(0.006, 0.02)
        var pts: [[Double]] = []
        for step in 0...9 {
            let angle = Double(step) / 9 * 2 * .pi
            let wobble = rng.double(0.7, 1.12)
            pts.append([lat + sin(angle) * radius * wobble, lon + cos(angle) * radius * wobble * 1.4])
        }
        return pts
    }
    ...
    private static let usCities: [(name: String, lat: Double, lon: Double)] = [
        ("Austin, TX",30.27,-97.74),("New York, NY",40.71,-74.01),("Los Angeles, CA",34.05,-118.24),
        ("Chicago, IL",41.88,-87.63),("Denver, CO",39.74,-104.99),("Seattle, WA",47.61,-122.33),
        ("Boston, MA",42.36,-71.06),("San Francisco, CA",37.77,-122.42),("Portland, OR",45.52,-122.68),
        ("Miami, FL",25.76,-80.19),("Boulder, CO",40.01,-105.27),("San Diego, CA",32.72,-117.16),
        ...
        ("Tampa, FL",27.95,-82.46),("Orlando, FL",28.54,-81.38),("Las Vegas, NV",36.17,-115.14),
        ...
        ("St. Louis, MO",38.63,-90.20),("Cincinnati, OH",39.10,-84.51),("New Orleans, LA",29.95,-90.07)]

    /// Non-US cities — the rest of the community.
    private static let worldCities: [(name: String, lat: Double, lon: Double)] = [
        ("London",51.51,-0.13),("Toronto",43.65,-79.38),("Sydney",-33.87,151.21),("Berlin",52.52,13.40),
        ("Paris",48.86,2.35),("Vancouver",49.28,-123.12),("Melbourne",-37.81,144.96),("Dublin",53.35,-6.26),
        ("Amsterdam",52.37,4.90),("Madrid",40.42,-3.70),("Tokyo",35.68,139.69),("Auckland",-36.85,174.76),
        ("Stockholm",59.33,18.06),("Mexico City",19.43,-99.13),("Barcelona",41.39,2.17),("Munich",48.14,11.58),
        ("Calgary",51.05,-114.07),("Cape Town",-33.92,18.42),("Singapore",1.35,103.82),("Oslo",59.91,10.75)]
