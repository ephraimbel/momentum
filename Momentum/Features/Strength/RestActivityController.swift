...
final class RestActivityController {
    ...
    /// `CardioActivityController.endOrphans`. A timer started during deferred startup is protected.
    static func endOrphans() {
        let orphans = Activity<RestActivityAttributes>.activities.filter { ownership.isOrphan($0.id) }
