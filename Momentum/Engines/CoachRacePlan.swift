...
enum CoachRacePlan {
    ...
    static func sections(profile: UserProfile, today: Date = Date(),
        ...
        let warmup = distanceM <= RaceDistance.tenK.meters
            ? "10 to 15 minutes easy jogging, then 4 strides at race effort, finishing about 10 minutes before the gun."
            : "5 to 10 minutes easy at most. The first kilometers are the warmup. Save every match for after halfway."
        ...
        if let carbs = fueling.carbsPerHour {
            fuelLine += " Aim for \(carbs.lowerBound) to \(carbs.upperBound) g of carbs per hour, starting early. Nothing new on race day."
        }
