...
// MARK: - Step results (what Pace Insights reviews)

/// The recorded outcome of one step — captured by the tracker as each step completes, persisted as
/// JSON on the workout at finish, and reviewed post-run by `PaceInsights` (achieved vs prescribed).
/// `kind` is the `WorkoutStep.Kind` rawValue, kept as a plain string for stable JSON.
struct StepResult: Equatable, Sendable, Codable {
    var kind: String
    var targetPaceSPerKm: Double?
    var toleranceSPerKm: Double
    var repIndex: Int?
    var repTotal: Int?
    /// Distance covered (m) and time spent (s) while the step was active.
    var distanceM: Double
    var durationS: Double
    var skipped: Bool

    /// Achieved pace (s/km); nil when the step covered too little ground to mean anything
    /// (a timed walk break, a step crossed in the same tick as its predecessor).
    var achievedPaceSPerKm: Double? {
        guard distanceM >= 50, durationS > 0 else { return nil }
...
struct StructuredRunTracker: Equatable, Sendable {
    ...
    /// One entry per finished (or skipped) step, in order — the raw material for `PaceInsights`.
    ...
    mutating func advance(distanceM: Double, elapsedS: Double) -> Bool {
        ...
        while let step = current, targetMet(step, distanceM: distanceM, elapsedS: elapsedS) {
            record(step, distanceM: distanceM, elapsedS: elapsedS, skipped: false)
            index += 1
    ...
    mutating func skip(distanceM: Double, elapsedS: Double) -> Bool {
        ...
        record(step, distanceM: distanceM, elapsedS: elapsedS, skipped: true)
        index += 1
        anchorDistanceM = distanceM
        anchorElapsedS = elapsedS
        return true
    ...
    /// Bank the finished step's achieved distance/time (anchor → now) for the post-run review.
    private mutating func record(_ step: WorkoutStep, distanceM: Double, elapsedS: Double, skipped: Bool) {
        results.append(StepResult(kind: step.kind.rawValue,
                                  targetPaceSPerKm: step.paceSPerKm,
                                  toleranceSPerKm: step.toleranceSPerKm,
                                  repIndex: step.repIndex, repTotal: step.repTotal,
                                  distanceM: distanceM - anchorDistanceM,
                                  durationS: elapsedS - anchorElapsedS,
                                  skipped: skipped))
    }
