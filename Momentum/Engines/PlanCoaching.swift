...
enum PlanCoaching {
    ...
    /// The consent-required inverse of `recalibratePaces`, offered by Pace Insights when a session's
    /// targets consistently ran hot ("Review"): ease the plan's assumed 5k a small bounded step and
    /// re-derive **future** planned running paces so the prescription stays honest. Never touches
    /// history; bounded to +2% per approval (mirrors recalibrate's ≤3% in the other direction) so a
    /// tap can't lurch the plan. No-shame: this is "make the targets fit", never a demotion.
    /// Returns the number of future sessions updated (0 ⇒ nothing upcoming to change).
    @discardableResult
